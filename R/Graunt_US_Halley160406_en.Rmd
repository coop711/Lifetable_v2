...
# dev.copy(device = png, file = "../pics/graunt.us93.png")
```
...
# dev.copy(device = png, file = "../pics/graunt_halley.us93_poly.png")
```
...
(gu6 <- gu5 + 
   scale_x_continuous(breaks = graunt$x) + 
   scale_y_continuous(breaks = graunt$xPo.g))
# ggsave("../pics/graunt.us_ggplot.png", gu6)
```
...
(gup7 <- gup6 + 
   scale_x_continuous(breaks = graunt$x) + 
   scale_y_continuous(breaks = graunt$xPo.g))
# ggsave("../pics/graunt.us_poly.png", gup7)
```
...
(ghu4 <- ghu3 + 
  ...
  scale_x_continuous(breaks = c(graunt$x, 84)) + 
  scale_y_continuous(breaks = c(graunt$xPo.g, xPo.halley.age.6)))
# ggsave("../pics/graunt_halley.us_ggplot.png", ghu4)
```
...
(ghup7 <- ghup6 +
  geom_point(data = data.frame(x = 84, y = halley$xPo[85]), aes(x = x, y = y),  colour = 3, shape = 21, fill = "white"))
# ggsave("../pics/graunt_halley.us_poly_ggplot.png", ghup7)
```
